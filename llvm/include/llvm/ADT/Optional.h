...
template <typename T> constexpr bool operator<(const Optional<T> &, std::nullopt_t) {
  return false;
}

template <typename T> constexpr bool operator<(std::nullopt_t, const Optional<T> &X) {
  return X.has_value();
}
...
template <typename T> constexpr bool operator>(const Optional<T> &X, std::nullopt_t) {
  return std::nullopt < X;
}
